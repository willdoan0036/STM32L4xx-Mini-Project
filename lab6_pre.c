...
void TimerSetup() {
	...
	SET_BIT(RCC->APB2ENR, RCC_APB2ENR_TIM7EN); //enable clock source
  TIM7->ARR = 0x333; //set auto reload. assumes 2MHz
  TIM7->PSC=0xFF; //set prescale. assumes 2MHz
  SET_BIT(TIM7->DIER, TIM_DIER_UIE); //enable interupts
}
...
void TIM7_IRQHandler() {
  ...
  count2 = (count2 + 1) % 10;
	GPIOB->BSRR = 0xF0000;
  SET_BIT(GPIOB->BSRR, count2 & 0xF); //turn on decimal
 
  if (count2 == 0) {
		count1 = (count1 + 1) % 10;
		GPIOB->BSRR = 0xF00000;
		SET_BIT(GPIOB->BSRR, count1 << 4);
	} 
